...
class SelectQueries:
    ...
    def _prepare_statements(self) -> None:
        ...
        self._prep('customer_by_id',
                   "SELECT * FROM customer WHERE c_id = ?")
        self._prep('account_by_id',
                   "SELECT * FROM customer_account WHERE ca_id = ?")
        self._prep('broker_by_id',
                   "SELECT * FROM broker WHERE b_id = ?")
        self._prep('security_by_symbol',
                   "SELECT * FROM security WHERE s_symb = ?")
        self._prep('last_trade_by_symbol',
                   "SELECT * FROM last_trade WHERE lt_s_symb = ?")
        self._prep('status_type_by_id',
                   "SELECT * FROM status_type WHERE st_id = ?")
        self._prep('trades_by_account',
                   "SELECT * FROM trade_by_account WHERE t_ca_id = ? LIMIT ?")
        self._prep('holdings_by_account',
                   "SELECT * FROM holding WHERE h_ca_id = ? LIMIT ?")
        self._prep('watch_items_by_watchlist',
                   "SELECT * FROM watch_item WHERE wi_wl_id = ?")
        self._prep('daily_market_range',
                   "SELECT * FROM daily_market WHERE dm_s_symb = ? "
                   "AND dm_date >= ? AND dm_date <= ?")
        self._prep('financial_by_company',
                   "SELECT * FROM financial WHERE fi_co_id = ? LIMIT ?")
        self._prep('holding_summary_by_account',
                   "SELECT * FROM holding_summary WHERE hs_ca_id = ?")
        self._prep('news_by_company',
                   "SELECT * FROM news_by_company WHERE nx_co_id = ? LIMIT ?")
        self._prep('trades_by_symbol_date_range',
                   "SELECT * FROM trade_by_symbol WHERE t_s_symb = ? "
                   "AND t_dts >= ? AND t_dts <= ? LIMIT ?")
        self._prep('active_trades_with_filter',
                   "SELECT * FROM trade_by_account WHERE t_ca_id = ? "
                   "AND t_st_id = ? LIMIT ? ALLOW FILTERING")
        self._prep('market_feed_latest',
                   "SELECT * FROM market_feed WHERE mf_s_symb = ? LIMIT ?")
        self._prep('holding_summary_for_portfolio',
                   "SELECT hs_s_symb, hs_qty FROM holding_summary WHERE hs_ca_id = ?")
        self._prep('last_trade_for_symbol',
                   "SELECT lt_price FROM last_trade WHERE lt_s_symb = ?")
        self._prep('broker_metrics',
                   "SELECT * FROM broker_metrics WHERE b_id = ?")
        self._prep('companies_by_industry',
    ...
    def select_daily_market_range(self, symbol: str,
                                  start_date: date, end_date: date) -> List[Any]:
    ...
    def select_companies_by_industry(self, industry_id: str, limit: int = 20) -> List[Any]:
        result = self.session.execute(
            self._prepared['companies_by_industry'], [industry_id, limit]
        )
        return list(result)

    def select_financial_by_company(self, company_id: int, limit: int = 8) -> List[Any]:
        result = self.session.execute(
            self._prepared['financial_by_company'], [company_id, limit]
        )
        return list(result)

    def select_holding_summary_by_account(self, account_id: int) -> List[Any]:
        result = self.session.execute(
            self._prepared['holding_summary_by_account'], [account_id]
        )
        return list(result)

    def select_news_by_company(self, company_id: int, limit: int = 10) -> List[Any]:
        result = self.session.execute(
            self._prepared['news_by_company'], [company_id, limit]
        )
        return list(result)

    # --- Complex SELECT (C1-C6) ---

    def select_trades_by_symbol_date_range(self, symbol: str,
                                           start_dts: datetime, end_dts: datetime,
                                           limit: int = 50) -> List[Any]:
        result = self.session.execute(
            self._prepared['trades_by_symbol_date_range'],
            [symbol, start_dts, end_dts, limit]
        )
        return list(result)
    ...
    def select_active_trades_with_filter(self, account_id: int,
                                         status_id: str, limit: int = 20) -> List[Any]:
        result = self.session.execute(
            self._prepared['active_trades_with_filter'],
            [account_id, status_id, limit]
        )
        return list(result)

    def select_market_feed_latest(self, symbol: str, limit: int = 10) -> List[Any]:
        result = self.session.execute(
            self._prepared['market_feed_latest'], [symbol, limit]
        )
        return list(result)
    ...
    def select_portfolio_value(self, account_id: int) -> Dict[str, Any]:
        ...
        for row in holdings:
            ...
            total_value += val
            positions.append({'symbol': row.hs_s_symb, 'qty': row.hs_qty,
                               'price': price, 'value': val})
        return {'account_id': account_id, 'total_value': total_value,
                'positions': positions}

    def select_broker_performance(self, broker_id: int) -> Dict[str, Any]:
        """Get broker performance metrics from counter table."""
        broker_rows = self.session.execute(
            self._prepared['broker_by_id'], [broker_id]
        )
        broker = broker_rows.one()
        metrics_rows = self.session.execute(
            self._prepared['broker_metrics'], [broker_id]
        )
