...
from data_generator.data_loader import DataLoader
...
@click.option('--config', '-c', default='config/cassandra_config.yaml', 
...
@cli.command()
@click.option('--cassandra-config', '-c', default='config/cassandra_config.yaml',
              help='Path to Cassandra config file')
@click.option('--benchmark-config', '-b', default='config/benchmark_config.yaml',
              help='Path to benchmark config file')
@click.option('--sample-only', is_flag=True, 
              help='Generate only a small sample of data for testing')
...
def run_benchmark(cassandra_config, benchmark_config, dry_run):
    ...
    try:
        ...
        runner = BenchmarkRunner(
            cassandra_config_path=cassandra_config,
...
def run_query(query_type, cassandra_config, iterations):
    ...
    try:
        ...
        for i in range(iterations):
            ...
            result = runner.query_executor.execute_query(query)
...
def cleanup(config, force):
    ...
    if not force:
        click.confirm(
            'This will DELETE ALL data in the TPC-C keyspace. Are you sure?',
            abort=True
        )
