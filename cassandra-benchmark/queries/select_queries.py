...
class SelectQueries:
    ...
    def _prepare_statements(self) -> None:
        ...
        # Simple queries
        self.get_warehouse_stmt = self.session.prepare("SELECT * FROM warehouse WHERE w_id = ?")

        self.get_customer_stmt = self.session.prepare(
            "SELECT * FROM customer WHERE c_w_id = ? AND c_d_id = ? AND c_id = ?"
        )

        self.get_item_stmt = self.session.prepare("SELECT * FROM item WHERE i_id = ?")
        ...
        self.get_orders_by_customer_stmt = self.session.prepare(
            "SELECT * FROM orders_by_customer WHERE o_c_id = ? AND o_w_id = ? AND o_d_id = ? LIMIT ?"  # noqa: E501
        )
        ...
        self.get_history_by_date_stmt = self.session.prepare(
            "SELECT * FROM history WHERE h_w_id = ? AND h_d_id = ? AND date_bucket = ? AND h_date >= ? AND h_date <= ?"  # noqa: E501
        )
    ...
    def select_customer_by_id(
        ...
        result = self.session.execute(
            self.get_customer_stmt, [warehouse_id, district_id, customer_id]
        )
        return [dict(row._asdict()) for row in result]
    ...
    def select_district_by_id(self, warehouse_id: int, district_id: int) -> List[Dict[str, Any]]:
        ...
        """
        result = self.session.execute(self.get_district_stmt, [warehouse_id, district_id])
        return [dict(row._asdict()) for row in result]

    # ========== MEDIUM SELECT QUERIES ==========

    def select_customers_by_district(
        self, warehouse_id: int, district_id: int, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """
        ...
        """
        result = self.session.execute(
            self.get_customers_by_district_stmt, [warehouse_id, district_id, limit]
        )
        return [dict(row._asdict()) for row in result]
    ...
    def select_stock_level(self, warehouse_id: int, item_id: int) -> List[Dict[str, Any]]:
        ...
        """
        result = self.session.execute(self.get_stock_stmt, [warehouse_id, item_id])
        return [dict(row._asdict()) for row in result]

    def select_orders_by_customer(
        self, customer_id: int, warehouse_id: int, district_id: int, limit: int = 20
    ) -> List[Dict[str, Any]]:
        """
        ...
        """
        result = self.session.execute(
            self.get_orders_by_customer_stmt, [customer_id, warehouse_id, district_id, limit]
        )
        return [dict(row._asdict()) for row in result]
    ...
    def select_order_lines(
        ...
        """
        result = self.session.execute(
            self.get_order_lines_stmt, [warehouse_id, district_id, order_id]
        )
        return [dict(row._asdict()) for row in result]

    # ========== COMPLEX SELECT QUERIES ==========

    def select_customers_by_name(
        self, warehouse_id: int, district_id: int, last_name: str
    ) -> List[Dict[str, Any]]:
        """
        ...
        """
        result = self.session.execute(
            self.get_customers_by_name_stmt, [warehouse_id, district_id, last_name]
        )
        return [dict(row._asdict()) for row in result]

    def select_new_orders(
        self, warehouse_id: int, district_id: int, limit: int = 20
    ) -> List[Dict[str, Any]]:
        """
        ...
        """
        result = self.session.execute(self.get_new_orders_stmt, [warehouse_id, district_id, limit])
        return [dict(row._asdict()) for row in result]

    def select_history_by_date_range(
        self, warehouse_id: int, district_id: int, date_bucket: str, start_date: str, end_date: str
    ) -> List[Dict[str, Any]]:
        """
    ...
    def select_customers_by_credit(
        self, credit_status: str, fetch_size: int = 100
    ) -> List[Dict[str, Any]]:
        """
    ...
    def select_orders_range(
        self, warehouse_id: int, district_id: int, start_order_id: int, end_order_id: int
    ) -> List[Dict[str, Any]]:
        """
        ...
        """
        result = self.session.execute(
            query, [warehouse_id, district_id, start_order_id, end_order_id]
        )
        return [dict(row._asdict()) for row in result]
    ...
    def select_customer_with_token(
        self, warehouse_id: int, district_id: int, token_value: int = None, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """
    ...
    def select_customer_projection(
        self, warehouse_id: int, district_id: int, customer_id: int
    ) -> Dict[str, Any]:
        """
    ...
    def select_order_lines_range(
        self, warehouse_id: int, district_id: int, order_id: int, start_line: int, end_line: int
    ) -> List[Dict[str, Any]]:
        """
        ...
        """
        result = self.session.execute(
            query, [warehouse_id, district_id, order_id, start_line, end_line]
        )
        return [dict(row._asdict()) for row in result]

    def select_orders_by_carrier_index(
        self, carrier_id: int, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """
    ...
    def select_districts_multi_warehouse(
        self, warehouse_ids: List[int], district_id: int
    ) -> List[Dict[str, Any]]:
        """
