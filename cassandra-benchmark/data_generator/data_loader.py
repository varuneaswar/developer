...
class DataLoader:
    ...
    def load_districts(self) -> int:
        ...
        for w_id in range(1, self.generator.num_warehouses + 1):
            for d_id in range(1, self.generator.num_districts_per_warehouse + 1):
                ...
                self.session.execute(self.insert_district, [
                    district['d_w_id'], district['d_id'], district['d_name'],
                    district['d_street_1'], district['d_street_2'], district['d_city'],
                    district['d_state'], district['d_zip'], district['d_tax'],
                    district['d_ytd'], district['d_next_o_id']
                ])
                count += 1
    ...
    def load_customers(self, batch_size: int = 50) -> int:
        ...
        """
        total = (self.generator.num_warehouses * 
            ...
            for d_id in range(1, self.generator.num_districts_per_warehouse + 1):
                for c_id in range(1, self.generator.num_customers_per_district + 1):
                    ...
                    customer_by_name_params.append([
                        ...
                        customer['c_first'], customer['c_id'], customer['c_middle'],
                        customer['c_street_1'], customer['c_street_2'], customer['c_city'],
                        customer['c_state'], customer['c_zip'], customer['c_phone'],
                        customer['c_since'], customer['c_credit'], customer['c_credit_lim'],
                        customer['c_discount'], customer['c_balance'], customer['c_ytd_payment'],
                    ...
                    if len(customer_params) >= batch_size:
                        execute_concurrent_with_args(
                            self.session, self.insert_customer, customer_params,
                            concurrency=batch_size
                        )
                        execute_concurrent_with_args(
                            self.session, self.insert_customer_by_name, customer_by_name_params,
                            concurrency=batch_size
                        )
    ...
    def load_items(self, batch_size: int = 100) -> int:
        ...
        for i_id in range(1, self.generator.num_items + 1):
            ...
            item_params.append([
                item['i_id'], item['i_im_id'], item['i_name'],
                item['i_price'], item['i_data']
            ])
            count += 1
            ...
            if len(item_params) >= batch_size:
                execute_concurrent_with_args(
                    self.session, self.insert_item, item_params,
                    concurrency=batch_size
                )
